...
### 2.1 入力機能 (UI刷新: 2026/02/22)
*   **ダイヤルピッカーの採用**: モバイル端末での操作性を重視し、従来の数値入力に代わりスクロール式のダイヤルピッカーを導入。
*   **年齢入力**: 「歳」と「ヶ月」の2つの独立したダイヤルで直感的に選択可能。
*   **体重入力**: 「十の位」と「一の位」の2つのダイヤルを使用。入力の簡略化と誤操作防止のため、小数点以下を廃止。
*   **省スペースレイアウト**: モバイル表示時、年齢・体重・標準体重ボタン・削除ボタンを1行に集約し、計算結果の表示面積を最大化。
*   **標準体重自動入力**: 年齢から推計された標準体重（整数値）をワンタップで反映。
