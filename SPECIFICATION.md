...
8.  **健康レシピ提案:** AIが症状や食材に合わせて健康レシピを提案する。
...
### 3.6. 健康レシピ提案 (`recipe-app/`)

#### 3.6.1. 機能概要

ユーザーの体調や手持ちの食材、希望する料理ジャンルに基づいて、AIが最適なレシピを提案する。

#### 3.6.2. 詳細仕様

詳細は `recipe-app/SPECIFICATION.md` を参照のこと。

#### 3.6.3. 主な機能

-   **症状入力**: 体調や気になる症状を選択・入力。
-   **条件設定**: 食材、ジャンル、調理時間を指定。
-   **AI提案**: OpenAIまたはGeminiを利用してレシピを生成。
-   **印刷機能**: 生成されたレシピを印刷用に整形して出力。
...
## 4. 今後の実装予定 (Future Plans)

プロジェクトの品質向上と保守性確保のため、以下の機能を順次追加する。

### 4.1. ログ収集の導入
- **エラー監視**: Sentry または Google Apps Script (GAS) を利用した、フロントエンドエラーの自動検知と通知。
- **利用状況分析**: Google Analytics 4 (GA4) を活用した、検索ワードや機能利用頻度の集計。
- **設計指針**: 詳細は [logging_policy.md](docs/logging_policy.md) を参照。

### 4.2. セキュリティとプライバシー
- ログ収集時の個人情報（PII）除外の徹底。
- プライバシーポリシーの更新とユーザーへの明示。
