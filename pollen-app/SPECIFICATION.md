...
*   **自動更新機能**:
    *   **毎時10分に自動更新**: 毎時10分になると自動的に最新の花粉データをAPIから取得し、表示を更新する。
    *   **次回更新時刻表示**: サイドパネルに次回更新予定時刻を表示。
    *   **カウントダウン表示**: 次回更新までの残り時間を秒単位でリアルタイム表示。
    *   **手動更新ボタン**: ユーザーが任意のタイミングで「今すぐ更新」ボタンをクリックすることで、即座にデータを再取得可能。
    *   **キャッシュクリア**: 更新時には全てのキャッシュをクリアし、最新データを確実に取得する。
    *   **視覚的フィードバック**: 更新中はボタンがアニメーション表示され、完了時にはトースト通知で結果を表示。
*   **トースト通知**:
...
*   **通知登録地点の表示**:
    *   アラート設定済みの地点をサイドパネル上部に表示。
    *   クリックすることで、地図上の該当地点へ即座にジャンプし、詳細情報を表示する。
